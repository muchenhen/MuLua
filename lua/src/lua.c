...
int main(int argc, char** argv)
{
    ...
    // 将pmain函数压入栈中。这个函数可能包含了Lua解释器的主要逻辑。
    lua_pushcfunction(L, &pmain);   
    // 将argc和argv压入栈中。这两个参数将会传递给pmain函数。
    lua_pushinteger(L, argc);       
    lua_pushlightuserdata(L, argv); 
    // 调用lua_pcall函数，传递参数个数为2，返回值个数为1，错误处理函数的索引为0。
    status = lua_pcall(L, 2, 1, 0); 
    // 从栈顶获取pmain函数的返回值，转换为布尔值
    result = lua_toboolean(L, -1);  
    // 报告lua_pcall的执行状态，可能会输出错误信息。
    report(L, status);

    lua_close(L); /* 关闭Lua状态，释放所有相关资源。*/
